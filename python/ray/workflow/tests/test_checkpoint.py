...
def _assert_step_checkpoints(wf_storage, step_id, mode):
    ...
    if mode == "all_skipped":
        ...
        assert (result.output_object_valid
                or result.output_step_id is not None)
    else:
...
def test_checkpoint_dag(workflow_start_regular):
    ...
    start = time.time()
    outputs = checkpoint_dag.options(
        name="checkpoint_dag",
        checkpoint=False).step(False).run(workflow_id="checkpoint_skip")
    run_duration_skipped = time.time() - start
    assert np.isclose(outputs, 8388607.5)

    wf_storage = workflow_storage.WorkflowStorage("checkpoint_skip",
                                                  global_storage)
    _assert_step_checkpoints(
        wf_storage, "checkpoint_dag", mode="output_skipped")
    ...
    start = time.time()
    outputs = checkpoint_dag.options(name="checkpoint_dag").step(False).run(
    ...
    start = time.time()
    outputs = checkpoint_dag.options(name="checkpoint_dag").step(True).run(

...
# For np 2D arrays of this size, it is about 2-3ms per add operation.
LEAF_SIZE = 2 ** 10
...
def matmul(A, B):
    ...
    M1 = matmul.remote(matsum.remote(A11, A22), matsum.remote(B11, B22))
    M2 = matmul.remote(matsum.remote(A21, A22), B11)
    M3 = matmul.remote(A11, matsum.remote(B12, True, B22))
    M4 = matmul.remote(A22, matsum.remote(B21, True, B11))
    M5 = matmul.remote(matsum.remote(A11, A12), B22)
    M6 = matmul.remote(matsum.remote(A21, True, A11), matsum.remote(B11, B12))
    M7 = matmul.remote(matsum.remote(A12, True, A22), matsum.remote(B21, B22))

    C = np.empty((A.shape[0], B.shape[1]))

    blocks = ray.get([
            matsum.remote(M1, M4, True, M5, M7),
            matsum.remote(M3, M5),
            matsum.remote(M2, M4),
            matsum.remote(M1, True, M2, M3, M6),
            ])

    assign_matrix(C, QUARTER_11, blocks[0])
    assign_matrix(C, QUARTER_12, blocks[1])
    assign_matrix(C, QUARTER_21, blocks[2])
    assign_matrix(C, QUARTER_22, blocks[3])

...
if [ -z "$GCS_DELAY_MS" ]; then
    ...
    ray start --head --huge-pages --plasma-directory /mnt/hugepages --redis-port=6379 --resources='{"Node0": 1000}' --num-cpus 100 --use-raylet --num-redis-shards $NUM_REDIS_SHARDS --use-task-shard --gcs-delay-ms $GCS_DELAY_MS
fi

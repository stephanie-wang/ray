...
def do_batch(f, opts, args=None, depth=0):
    ...
    results = [f.remote([arg], opts, depth=depth) for arg in args]
    ray.get(results)
    return results


@ray.remote
def f(arg, opts, depth=0):
    if depth == TREE_DEPTH:
        if isinstance(arg[0], ray.ObjectID):
...
def main(opts):
    ...
    timeit(chained_batch, multiplier=TASKS_PER_BATCH**TREE_DEPTH)

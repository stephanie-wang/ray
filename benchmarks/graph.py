...
@ray.remote
def ancestor(graph_id, node_index):
    graph = ray.get(graph_id[0])
    node = graph[node_index]
    if node.parent is None:
        return [node.index]

    ancestors = ray.get(ancestor.remote(graph_id, node.parent))
    return [node.index] + ancestors

@ray.remote
def ancestors(graph_id, node_indices):
    return [ancestor.remote(graph_id, node_index) for node_index in node_indices]

@ray.remote
def get_leaves(graph_id, node_index):
    graph = ray.get(graph_id[0])
    node = graph[node_index]
    if len(node.children) == 0:
        return [node.index]

    children_leaves = ray.get([get_leaves.remote(graph_id, child) for child in node.children])
...
if __name__ == '__main__':
    ...
    for _ in range(NUM_TRIALS):
        ...
        leaves = [get_leaves.remote([tree_id], 0)]
        path_id_lists = ray.get([ancestors.remote([tree_id], leaf) for leaf in leaves])
